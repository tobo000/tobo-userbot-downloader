...
class ErrorNotifier:
    ...
    def album_report(self, album_id: str, title: str, photos: int, videos: int, 
        ...
        border = "=" * 60
        status = "✅ COMPLETE + GITHUB SYNCED" if (success and missing_p == 0 and missing_v == 0 and github_synced) else "❌ ISSUES FOUND"
        report = (
            f"\n{border}\n"
        ...
        report += f"\nGitHub: {'✅ Synced' if github_synced else '❌ Not Synced'}"
        report += f"\n{border}\n"
        print(report)
        logger.info(report)
...
class MediaTracker:
    ...
    def is_fully_uploaded(self, album_id: str) -> bool:
        """Check if ALL media has been uploaded successfully"""
        if album_id not in self.pending_albums:
            return False
        album = self.pending_albums[album_id]
        total_downloaded = len(album['downloaded']['photos']) + len(album['downloaded']['videos'])
        total_uploaded = len(album['uploaded']['photos']) + len(album['uploaded']['videos'])
        return total_downloaded > 0 and total_downloaded == total_uploaded
...
def download_from_github():
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            print(f"📡 GitHub Restore: No existing database - starting fresh")
    except Exception as e:
        print(f"❌ GitHub Restore: Error - {e}")

# ============================================
# DATABASE LOGIC (WITH FULLY PROCESSED TRACKING)
# ============================================
def init_db():
    """Initialize database"""
    ...
    conn.commit()
    conn.close()
    print("✅ Database initialized (fully_processed + failed_albums tracking)")

def is_fully_processed(album_id: str) -> bool:
    """Check if album was FULLY processed (downloaded + uploaded successfully)"""
    conn = sqlite3.connect(DB_NAME)
    res = conn.execute("SELECT 1 FROM fully_processed WHERE album_id = ?", (album_id,)).fetchone()
    conn.close()
    return res is not None

def mark_fully_processed(album_id: str, title: str, photos_count: int, videos_count: int):
    """Mark album as FULLY processed ONLY when download + upload both successful"""
    try:
        conn = sqlite3.connect(DB_NAME)
        ...
        github_synced = backup_to_github()
        print(f"   ✅ Fully Processed + GitHub: {album_id}")
        return github_synced
    except Exception as e:
        print(f"   ❌ mark_fully_processed error: {e}")
        return False
...
def mark_failed(album_id: str, url: str, title: str, photos_count: int, videos_count: int, error_type: str, error_message: str):
    ...
    try:
        ...
        # Sync to GitHub so new VPS can retry
        backup_to_github()
        print(f"   ⚠️  Marked as failed + GitHub: {album_id}")
    except:
        pass
...
# ============================================
# CORE DELIVERY (With Fully Processed + Failed Tracking + GitHub Sync)
# ============================================
async def process_album(client, chat_id, reply_id, url, username, current, total):
    """Process album - ONLY marks fully processed if ALL media uploaded + GitHub synced"""
    album_id = url.rstrip('/').split('/')[-1]
    
    # Check if FULLY processed (only if all media was uploaded successfully)
    if is_fully_processed(album_id):
        print(f"⏭️  Skipping (fully processed + GitHub synced): {album_id}")
        return True
    ...
    if photos:
        for i, p_url in enumerate(photos, 1):
            ...
            try:
                ...
                downloaded_photos.append((path, f"Photo `{i}/{len(photos)}` | `{size_h}`", p_url))
            except Exception as e:
    ...
    if videos:
        for v_idx, v_url in enumerate(videos, 1):
            ...
            try:
                ...
                downloaded_videos.append((filepath, thumb, w, h, dur, f"{media_type} `{v_idx}/{len(videos)}` | `{time.strftime('%M:%S', time.gmtime(dur))}` | `{size_h}`", is_gif, v_url))
            except Exception as e:
    ...
    # ============================================
    # PHASE 2: UPLOAD (Sequential - WITH LOCK)
    # ============================================
    ...
    async with chat_lock:
        ...
        master_caption = f"**{title}**\n━━━━━━━━━━━━━━━━\nAlbum: `{current}/{total}`\nContent: `{len(downloaded_photos)}` Photos | `{len(downloaded_videos)}` Videos{' | ' + str(gif_count) + ' GIFs' if gif_count > 0 else ''}\nUser: `{username.upper()}`\nQuality: Original\n━━━━━━━━━━━━━━━━"
            ...
            for i in range(0, len(photo_media), 10):
                for attempt in range(3):
                    try:
                        ...
                        await asyncio.sleep((e.value if hasattr(e, 'value') else 15) + 5)
                    except Exception as e:
    ...
    # ============================================
    # PHASE 3: VERIFY & MARK (Only if ALL uploaded)
    # ============================================
    missing = media_tracker.get_missing_media(album_id)
    missing_p, missing_v = len(missing['photos']), len(missing['videos'])
    
    # Only mark as fully processed if ALL media uploaded AND no missing
...
async def retry_failed_albums(client, chat_id, reply_id):
    ...
    if failed:
        print(f"\n{'='*60}")
        print(f"🔄 RETRYING {len(failed)} FAILED ALBUMS (from GitHub)")
        print(f"{'='*60}")
        
        for album in failed:
            # Check if already fully processed (maybe fixed by another VPS)
            if is_fully_processed(album['album_id']):
                print(f"   ✅ Already processed by another instance: {album['album_id']}")
                continue
...
async def main():
    ...
    async with app:
        ...
        print("✅ Upload: Sequential (no mixing)")
        print("✅ GitHub Sync: Only when FULLY processed")
        print("✅ Failed Tracking: Auto-retry on restart")
        print("✅ Scanner: ALL pages")
        print("✅ GIF→MP4 | Compression | Anti-Flood")
        print("=" * 60)
        print("Commands: .user | .retry | .failed | .dashboard | .errors | .missing | .stats | .cancel | .reset")
        print("=" * 60 + "\n")
