...
async def process_album(client, chat_id, reply_id, url, username, current, total):
    ...
    if photos:
        for i in range(0, len(photos), 10):
            ...
            chunk = photos[i:i+10]
            photo_group = []
            for p_idx, p_url in enumerate(chunk):
                current_p_idx = i + p_idx
                path = os.path.join(user_folder, f"p_{current_p_idx}.jpg")
                try:
        ...
        for v_idx, v_url in enumerate(videos, 1):
            ...
            try:
                ...
                if os.path.exists(final_v): 
                    os.remove(filepath); os.rename(final_v, filepath)

                dur, w, h = get_video_meta(filepath)
                real_v_size = get_human_size(os.path.getsize(filepath))
                thumb = filepath + ".jpg"
            ...
            try:
                ...
                for aid in list(dict.fromkeys(ids)):
                    ...
                    if f_url not in all_urls:
                        ...
                        if block:
                            pm = re.search(r'(\d+) photos', block.group())
                            vm = re.search(r'(\d+) videos', block.group())
...
async def main():
    init_db()
    async with app:
        print("LOG: Full Finalized Bot Started!")
        await idle()
