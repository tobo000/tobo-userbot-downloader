...
# --- 2. HELPERS (Progress Bar & Metadata) ---
...
# --- 3. DOWNLOAD ENGINE (Nitro + Progress Animation) ---
...
# ==========================================
# SCRAPER ENGINE (V8.87: Improved Discovery)
# ==========================================
...
# ==========================================
# DELIVERY ENGINE (The Core Logic)
# ==========================================
        ...
        for v_idx, v_url in enumerate(videos, 1):
            ...
            try:
                ...
                if size > 15*1024*1024:
                    await status.edit_text(f"📥 **[{current}/{total}]** Downloading Video {v_idx}/{len(videos)} (Nitro)...")
                ...
                # Add to Batch
                v_payload.append(InputMediaVideo(filepath, thumb=thumb if os.path.exists(thumb) else None, width=w, height=h, duration=dur, caption=f"🎬 **{title}**\n📦 {get_human_size(size)}", supports_streaming=True))
                v_paths.append((filepath, thumb))

                # Upload Batch (Max 10)
                if len(v_payload) == 10 or v_idx == len(videos):
                    await status.edit_text(f"📤 **[{current}/{total}]** Uploading Video Batch...")
                    try:
                        await client.send_media_group(chat_id, v_payload, reply_to_message_id=reply_id, message_thread_id=topic_id)
                    except:
                        for vp, th in v_paths:
                            meta = get_video_meta(vp)
                            await client.send_video(chat_id, vp, thumb=th, width=meta[1], height=meta[2], duration=meta[0], supports_streaming=True, reply_to_message_id=reply_id, message_thread_id=topic_id)
...
async def main():
    init_db()
    async with app:
        print("LOG: V8.87 Final Integrated Edition Online!")
        await idle()
