                ...
                for chunk in r.iter_content(chunk_size=1024*1024):
                    if chunk:
                        ...
                        if now - last_update[0] > 6:
                            bar = create_progress_bar(downloaded, int(r.headers.get('content-length', downloaded)))
...
async def process_album(client, message, url, username, current, total):
    ...
    # [FIX] Force peer resolve to prevent stuck
        ...
        for i, v_url in enumerate(videos, 1):
            ...
            try:
                ...
                if size > 15*1024*1024:
                    await edit_status_safe(status, f"📥 **[{current}/{total}]** Nitro Downloading..."); download_nitro(v_url, filepath, headers, size)
                else:
...
async def main():
    init_db()
    async with app:
        print("LOG: V9.00 Final (Peer Fix & Nitro) Online!")
        await idle()
