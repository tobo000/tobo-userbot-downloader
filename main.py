...
def backup_to_github():
    ...
    try:
        ...
        print("LOG: [↑] Database synced to GitHub.")
    except Exception as e: print(f"GH Backup Error: {e}")
...
def download_from_github():
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            print("LOG: [↓] Latest Database restored from GitHub.")
        else: print("LOG: No existing database found on GitHub. Starting fresh.")
    except Exception as e: print(f"GH Download Error: {e}")

# --- 3. DATABASE LOGIC ---
def init_db():
    download_from_github() # ទាញយកទិន្នន័យចាស់មកវិញពេលចាប់ផ្ដើម
...
def mark_media_processed(media_url, album_id):
    ...
    try:
        ...
        conn.commit(); conn.close()
        # Update ទៅ GitHub រៀងរាល់ពេលផ្ញើរូប/វីដេអូចប់ ដើម្បីការពារការបាត់បង់
        backup_to_github()
    except: pass
...
async def safe_edit(msg, text):
    try: await msg.edit_text(text)
        ...
        await asyncio.sleep(e.value); await msg.edit_text(text)
    except: pass
...
def scrape_album_details(url):
    headers = {'User-Agent': 'Mozilla/5.0 Chrome/121.0.0.0', 'Referer': 'https://www.erome.com/'}
    try:
        res = session.get(url, headers=headers, timeout=20); soup = BeautifulSoup(res.text, 'html.parser')
...
async def process_album(client, chat_id, reply_id, url, username, current, total):
    ...
    if pending_p:
        for i in range(0, len(pending_p), 10):
            ...
            for p_url in chunk:
                p_path = os.path.join(user_folder, f"p_t_{pending_p.index(p_url)}.jpg")
                try:
                    r = session.get(p_url, timeout=15); size_h = get_human_size(len(r.content))
                    with open(p_path, 'wb') as f: f.write(r.content)
                    cap = album_cap if not sent_any and all_photos.index(p_url) == 0 else f"🖼 Photo: `{all_photos.index(p_url)+1}/{len(all_photos)}` | 📦 Size: `{size_h}`"
    ...
    for v_idx, v_url in enumerate(all_videos, 1):
        ...
        try:
            ...
            try:
                ...
                mark_media_processed(v_url, album_id); sent_any = True
            except: pass
            await asyncio.sleep(2); os.remove(filepath); os.remove(thumb)
        except: pass
...
async def user_cmd(client, message):
    ...
    query = raw_input.split("erome.com/")[-1].split('/')[0]; msg = await message.reply(f"🛰 **Scanning for `{query}`...**")
    ...
    for base_url in scan_targets:
        ...
        while page <= 15:
            ...
            try:
                ...
                res = session.get(f"{base_url}?page={page}" if "?" in base_url else f"{base_url}?page={page}", headers={'User-Agent': 'Mozilla/5.0'}, timeout=15); soup = BeautifulSoup(res.text, 'html.parser')
                ...
                for album in albums:
                    ...
                    if link_tag:
                        ...
                        if u not in all_urls:
                            all_urls.append(u); p_t = album.select_one('span.album-photos'); v_t = album.select_one('span.album-videos')
...
async def cancel_cmd(client, message):
    ...
    backup_to_github() # Sync ទៅ GitHub ភ្លាមពេលចុច Cancel
    await message.reply("🛑 **Cancelled and Progress Synced to GitHub!**")
...
async def reset_db(client, message):
    ...
    backup_to_github(); await message.reply("🧹 **Memory Cleared Everywhere!**")

async def main():
    init_db() # វានឹង Download Database ចុងក្រោយពី GitHub មកប្រើដោយស្វ័យប្រវត្តិ
    async with app:
        print("LOG: Elite Sync Bot Started!"); await idle()
