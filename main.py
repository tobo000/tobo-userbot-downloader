...
async def process_album(client, chat_id, reply_id, url, username, current, total):
        ...
        for i in range(0, len(pending_p), 10):
            ...
            for p_url in chunk:
                ...
                try:
                    ...
                    p_info = f"🖼 Photo: {p_idx_total+1}/{len(all_photos)} | 📦 {get_human_size(os.path.getsize(p_path))}"
                    cap = master_cap_base + p_info if media_sent_count == 0 else p_info
            ...
            for f in glob.glob(os.path.join(user_folder, "p_t_*.jpg")): os.remove(f)

    # --- VIDEOS (Final Playable Fix) ---
    for v_idx, v_url in enumerate(all_videos, 1):
        ...
        try:
            ...
            # ប្រើ libx264, yuv420p និង faststart ដើម្បីឱ្យ Play ក្នុង Telegram បានភ្លាមៗ
            ...
            v_info = f"🎬 Video: {v_idx}/{len(all_videos)} | 📦 {size_h}"
            v_cap = master_cap_base + v_info if media_sent_count == 0 else v_info
...
async def user_cmd(client, message):
    ...
    # ស្កេនទាំងក្នុង Profile និង Search ដើម្បីកុំឱ្យបាត់អាល់ប៊ុម
    ...
    await message.reply(f"🏆 Archive Complete for `{query}`!")
...
async def main():
    init_db(); 
    async with app:
        print("LOG: V8.95 Master Online!"); await idle()
